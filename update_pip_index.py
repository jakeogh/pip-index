...
Example:
    update_pip_index.py eprint 0.0.1760909962 abc123def456 --index-repo ~/pip-index
...
def update_index(
    ...
    # Add new version (or update if version already exists)
    new_version = (version, commit_hash, github_user, github_repo)

    # Remove old entry with same version if exists
    versions = [v for v in versions if v[0] != version]
    versions.append(new_version)

    # Sort by version (as strings, which works for timestamp versions)
...
def main():
    ...
    parser.add_argument(
        "--index-repo",
        default=str(Path.home() / "pip-index"),
        help="Path to pip-index repository (default: ~/pip-index)",
    )
    parser.add_argument(
        "--github-user",
        default="jakeogh",
        help="GitHub username (default: jakeogh)",
    )
    parser.add_argument(
